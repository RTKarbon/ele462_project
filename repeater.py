...
def genSpiceModel(fname_sp, wire_line, technology, freq):
    ...
    file_sp.write('VSW 1 0 PULSE (0V vdd 0.1ns 0.1ns 0.1ns %es %es)\n' % (period/2, period))
    ...
    for match in all_matches:
        ...
        if m != None:
            try:
                ...
                if val < 0:
                    print "*** Warning ***"
                    print '    Negative rising time!'
                else:
                    tp_lh_list.append(val)
            except:
                print "*** Warning ***"
...
def processData(fname_csv):
    ...
    #############################################################################
    # Temporary code for searching an optimal size and length
    if EXAUSTIVE_SEARCH:
        tmp_wire = insertInverters(wire_line, opt_num_ideal_stages, opt_inv_size, technology)
        base_delay = getWireDelay(tmp_wire, technology, freq)
        print "Delay for calculated INV size: %e" % base_delay 
        curr_delay = base_delay
        print "\nGoing up:\n"
        curr_size = opt_inv_size
        while True:
            curr_size += 1
            print "trying size %d" % curr_size
            tmp_wire = insertInverters(wire_line, opt_num_ideal_stages, curr_size, technology)
            tmp_wire_delay = getWireDelay(tmp_wire, technology, freq)
            print "delay for this size: %e" % tmp_wire_delay
            if tmp_wire_delay > curr_delay:
                opt_up_size = curr_size - 1
                opt_up_delay = curr_delay
                break
            else:
                curr_delay = tmp_wire_delay
        ...
        while True:
            ...
            print "delay for this size: %e" % tmp_wire_delay
            if tmp_wire_delay > curr_delay:
                opt_down_size = curr_size + 1
                opt_down_delay = curr_delay
                break
            else:
                curr_delay = tmp_wire_delay

        opt_size = opt_down_size if opt_down_delay < opt_up_delay else opt_up_size
        print "\nIncreasing number of stages\n"
        curr_num_stages = opt_num_ideal_stages
        curr_delay = base_delay
        while True:
            curr_num_stages += 1
            print "Trying number of stages: %d, size %d" % (curr_num_stages, opt_size)
            tmp_wire = insertInverters(wire_line, curr_num_stages, opt_size, technology)
            tmp_wire_delay = getWireDelay(tmp_wire, technology, freq)
            print "delay: %e" % tmp_wire_delay
            if tmp_wire_delay > curr_delay:
                up_size_delay = curr_delay
                up_stages = curr_num_stages - 1
                break
            else:
                curr_delay = tmp_wire_delay

        print "\nDecreasing number of stages\n"
        curr_num_stages = opt_num_ideal_stages
        curr_delay = base_delay
        while True:
            curr_num_stages -= 1
            ...
            print "delay: %e" % tmp_wire_delay
            if tmp_wire_delay > curr_delay:
                down_size_delay = curr_delay
                down_stages = curr_num_stages + 1
                break
            else:
                curr_delay = tmp_wire_delay
        opt_num_stages = down_stages if down_size_delay < up_size_delay else up_stages

        # end of the code for temporary wire delay
        #############################################################################

        opt_num_ideal_stages = opt_num_stages
        opt_inv_size = opt_size
...
def main():
    ...
    for dir_name in args:
        ...
        print "Looking for data files in %s" % dir_name
        # dir_name = 'model_data_all_32_unisize' #'model_data_N2000_2mm_width'
